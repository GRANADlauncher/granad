...
class Material2D:
    ...
    _materials = {
        "graphene" : {
            "orbitals": [
                { "position" : (0,0), "shift" : (0,0,0), "attributes" : (0,1,0,0,"C"), "tag" : "sublattice_1" },
                { "position" : (-1/3,-2/3), "shift" : (0,0,0), "attributes" : (0,1,0,0,"C"), "tag" : "sublattice_2" }
            ],
            "lattice_basis": [
            ...
            "coulomb": {  ((0,1,0,0,"C"), (0,1,0,0,"C")) : ([16.522, 8.64, 5.333],lambda d: 14.399/d+0j)  },
    ...
    def _get_positions_in_lattice( self, positions_in_unit_cell, coefficients ):
        ...
        return self.lattice_constant * (coefficients @ self.lattice_basis + shift[:,None,:]).reshape(shift.shape[0]*coefficients.shape[0], 3)
    ...
    def cut_orbitals( self, polygon_vertices, plot = False, minimum_neighbor_number : int = 2 ):
        ...
        initial_atom_positions = self._get_positions_in_lattice( unit_cell_fractional_atom_positions, coefficients )

        # for 1d systems, we need neither pruning nor polygon cutting
        if sum(jnp.count_nonzero(self.lattice_basis, axis = 1)) == 1:
            if plot:
                _display_lattice_cut( polygon_vertices, initial_atom_positions, initial_atom_positions )                
    ...
    def _get_orbital_list( self, final_atom_positions = None, coefficients = None ):
        ...
        for attributes, orbital_properties_list in orbitals_by_quantum_numbers.items():
            ...
            for orbital_property in orbital_properties_list:
                plane_orbital_positions = self._get_positions_in_lattice( jnp.array([orbital_property["position"]]), coefficients )
                if final_atom_positions is not None:
                    plane_orbital_positions = self._keep_matching_positions( final_atom_positions, plane_orbital_positions)
