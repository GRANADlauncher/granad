...
def get_polygon(n, edge_type, side_length: float, min_side_length: float, side_length_increment: float, shift_vec_for_edge_alignment) -> jax.Array:
    """
    ...
    Parameters:
        ...
        edge_type (Literal['AC','ZZ']): Specifies Arm Chair or Zig Zag type edge.
    ...
    k=jnp.abs(side_length-min_side_length)//side_length_increment
    side_length=min_side_length+k*side_length_increment
    radius=side_length/(2*jnp.cos(jnp.pi/2*(1-2/n)))
    if (edge_type,n) in [('AC',6), ('ZZ',3)] : 
        polygon=jax.vmap(lambda theta : jnp.array([radius*jnp.cos(theta),radius*jnp.sin(theta),0]))(jnp.linspace(0,2*jnp.pi,n+1)[:-1])
    elif (edge_type,n) in [('ZZ',6), ('AC',3)] :
        polygon=jax.vmap(lambda theta : jnp.array([radius*jnp.sin(theta),radius*jnp.cos(theta),0]))(jnp.linspace(0,2*jnp.pi,n+1)[:-1])
    #The created polygon may not be on top of the sheet of material
...
def _cut_flake_graphene(polygon_id, edge_type, side_length, lattice_constant):
    ...
    a = 1
    mapping = {
    "triangle_armchair" : ["AC", 3, 6*a, 3*a, lambda k: jnp.array([1,0,0])*a],
    "triangle_zigzag" : ["ZZ", 3, 3*jnp.sqrt(3)*a, jnp.sqrt(3)*a, lambda k: jnp.array([0,0,0])*a],
    "hexagon_armchair" : ["AC", 6, 4*a, 3*a, lambda k: jnp.array([0,jnp.sqrt(3)/4*(1-(-1)**k),0])],
    "hexagon_zigzag" : ["ZZ", 6, (jnp.sqrt(3)+2/jnp.sqrt(3))*a, jnp.sqrt(3)*a, lambda k: jnp.array([0 ,(1-3*(-1)**k)*a/(4*jnp.sqrt(3)),0])]                
                }

    edge_type, n, min_side_length, side_length_increment, shift_vec_for_edge_alignment = mapping[f"{polygon_id}_{edge_type}"]
    polygon=get_polygon(n, edge_type, side_length, min_side_length, side_length_increment, shift_vec_for_edge_alignment)     
    path = Path(polygon[:,:2])
    m=jnp.int32((jnp.max(polygon[:,0])-jnp.min(polygon[:,0]))/(3*a)+1)
    n=jnp.int32((jnp.max(polygon[:,1])-jnp.min(polygon[:,1]))/jnp.sqrt(3*a)+1)
    ...
    return m, n, lattice_constant * polygon, lattice_constant * sheet[inside], lattice_constant * sheet
