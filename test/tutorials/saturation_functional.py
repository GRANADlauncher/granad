...
relaxation_function = granad.lindblad(
    stack, gamma_matrix.T, saturation=lambda x: 1 / (1 + jnp.exp(-1e6 * (2.0 - x)))
)
...
occupations = jnp.diagonal(
    stack.eigenvectors.conj().T @ sol.ys @ stack.eigenvectors, axis1=1, axis2=2
).real
...
for i, occ in enumerate(occupations.T):
    plt.plot(
        saveat * gamma, stack.electrons * occ.real, label=round(stack.energies[i], 2)
    )
plt.legend()
plt.show()
plt.close()
