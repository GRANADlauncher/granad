...
def plot_results( electrons, interpolate = False   ):
    ...
    for component in range(1):
        if interpolate:
            dip = jnp.interp( jnp.linspace(saveat.min(), saveat.max(), 2*saveat.size), saveat, dipole_moment[:, component] )
        else:
        ...
        omegas_ref = load_ref()[0]
        lower, upper = np.min(omegas_ref), np.max(omegas_ref) 
...
def sim_td( stack, tau ):

    # simulation duration
    time_axis = jnp.linspace(0, int(100), int(1e5))
    saveat = time_axis[::10]
    print(saveat.size)
    ...
    dipole_moment = granad.induced_dipole_moment(stack, jnp.diagonal(sol.ys, axis1 = 1, axis2 = 2) )
    ...
    plot_results_real_space( stack.electrons )
...
def run_sim( length ):
    ...
    for tau in [5]:# jnp.linspace(0.1, 10, 10):
        sim_td(stack, tau)
        # sim_rpa(stack, tau)
        
if __name__ == '__main__':
    # plot_results_real_space(1080)
    # 1/0
